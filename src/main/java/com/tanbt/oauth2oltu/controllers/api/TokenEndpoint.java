...
public class TokenEndpoint {
    ...
    public String authorize(@Context HttpServletRequest request)
        ...
        try {
            ...
            if (!TestContent.CLIENT_SECRET.equals(oauthRequest.getParam(OAuth.OAUTH_CLIENT_SECRET))) {
                OAuthResponse response =
                        OAuthASResponse.errorResponse(HttpServletResponse.SC_BAD_REQUEST)
                                .setError(OAuthError.TokenResponse.INVALID_CLIENT).setErrorDescription("client_secret invalid")
                                .buildJSONMessage();
            ...
            if (oauthRequest.getParam(OAuth.OAUTH_GRANT_TYPE)
                ...
                if (!TestContent.REFRESH_TOKEN.equals(oauthRequest
                        .getRefreshToken())) {
                    OAuthResponse response = OAuthASResponse
                            .errorResponse(HttpServletResponse.SC_BAD_REQUEST)
                            .setError(OAuthError.TokenResponse.INVALID_GRANT)
            ...
            OAuthResponse res = OAuthASResponse.errorResponse(
                    HttpServletResponse.SC_BAD_REQUEST).error(e)
                .buildJSONMessage();
            return res.getBody();
        }
    }
    ...
    @RequestMapping(value = "/api/token", method = RequestMethod.GET,
            consumes = "application/x-www-form-urlencoded", produces = "application/json")
    public String authorizeGet(@Context HttpServletRequest request) throws
